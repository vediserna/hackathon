...
- (void)newContact:(CDVInvokedUrlCommand*)command
{
		...
		OnymosContacts* __weak weakSelf = self;  // play it safe to avoid retain cycles
...
- (void)selectContact:(CDVInvokedUrlCommand *)command
{
...
- (void)searchContacts:(CDVInvokedUrlCommand*)command
{
		NSString* callbackId = command.callbackId;
		...
		[self.commandDelegate runInBackground:^{
				...
				[abHelper createAddressBook: ^(ABAddressBookRef addrBook, CDVAddressBookAccessError* errCode) {
						...
						if (![findOptions isKindOfClass:[NSNull class]]) {
								...
								hasPhoneNumber = [[findOptions objectForKey:@"hasPhoneNumber"] boolValue];
								startPosition = [[findOptions objectForKey:@"start"] intValue];
								limit = [[findOptions objectForKey:@"numberOfRecords"] intValue];
						}
						...
						if (!filter || [filter isEqualToString:@""]) {
								// get all records
								CFArrayRef allContacts = ABAddressBookCopyArrayOfAllPeople(addrBook);
								CFMutableArrayRef allContactsMutable = CFArrayCreateMutableCopy(kCFAllocatorDefault,
																																					 CFArrayGetCount(allContacts),
																																					 allContacts);

								CFArraySortValues(allContactsMutable,
																	CFRangeMake(0, CFArrayGetCount(allContactsMutable)),
																	(CFComparatorFunction) ABPersonComparePeopleByName,
																	(void*) ABPersonGetSortOrdering());

								foundRecords = [(__bridge_transfer NSArray*)allContactsMutable copy];
								CFRelease(allContacts);
								if (foundRecords && ([foundRecords count] > 0)) {
										// create Contacts and put into matches array
										// doesn't make sense to ask for all records when multiple == NO but better check
										xferCount = multiple == YES ? (int)[foundRecords count] : 1;
										if (xferCount > limit) {
											xferCount = limit;
										}
										...
										*/
										...
										recordCounter = 0;
										copiedCounter = 0;
										while ((recordCounter < (int)[foundRecords count]) && (copiedCounter < (startPosition + xferCount))) {
											CDVContact* xferContact = [[CDVContact alloc] initFromABRecord:(__bridge ABRecordRef)[foundRecords objectAtIndex:recordCounter]];

											if (hasPhoneNumber == YES) {
												ABMultiValueRef phoneRecord =(__bridge ABMultiValueRef)((__bridge NSString*)ABRecordCopyValue((__bridge ABRecordRef)[foundRecords objectAtIndex:recordCounter], kABPersonPhoneProperty));
												if (ABMultiValueGetCount(phoneRecord) > 0) {
													if (copiedCounter >= startPosition && copiedCounter < (startPosition + xferCount)) {
														[matches addObject:xferContact];
													}
													copiedCounter++;
												}
											...
											recordCounter++;
											xferContact = nil;
										} /* end while copiedCounter < (startPosition + xferCount) */
								}
						} else {
								CFArrayRef allContacts = ABAddressBookCopyArrayOfAllPeople(addrBook);
								CFMutableArrayRef allContactsMutable = CFArrayCreateMutableCopy(kCFAllocatorDefault,
								...
								*/
								...
								if (foundRecords && ([foundRecords count] > 0)) {
										...
										copiedCounter = 0;
										while ((recordCounter < (int)[foundRecords count]) && (copiedCounter < (startPosition + xferCount))) {
											CDVContact* possibleContact = [[CDVContact alloc] initFromABRecord:(__bridge ABRecordRef)[foundRecords objectAtIndex:recordCounter]];
											if (possibleContact) {
												bFound = [possibleContact foundValue:filter inFields:searchFields];
												if (bFound) {
													if (hasPhoneNumber == YES) {
														ABMultiValueRef phoneRecord =(__bridge ABMultiValueRef)((__bridge NSString*)ABRecordCopyValue((__bridge ABRecordRef)[foundRecords objectAtIndex:recordCounter], kABPersonPhoneProperty));
														if (ABMultiValueGetCount(phoneRecord) > 0) {
															if (copiedCounter >= startPosition && copiedCounter < (startPosition + xferCount)) {
																[matches addObject:possibleContact];
															}
															copiedCounter++;
														}
													}
						...
						NSString* searchTerm = (NSString*)[findOptions objectForKey:@"filter"]; 
						NSMutableDictionary *searchTermDict = [NSMutableDictionary dictionary];

						[searchTermDict setObject:searchTerm forKey: @"displayName"];
						[returnContacts addObject:searchTermDict];
