...
- (BOOL)foundValue:(NSString*)testValue inFields:(NSDictionary*)searchFields
{
    ...
    if ([searchFields valueForKey:kW3ContactGivenName]) {
        bFound = [self testStringValue:testValue forW3CProperty:kW3ContactGivenName];
        if (bFound == YES) {
            return bFound;
        }
    }

    if ([searchFields valueForKey:kW3ContactFamilyName]) {
        bFound = [self testStringValue:testValue forW3CProperty:kW3ContactFamilyName];
        if (bFound == YES) {
            return bFound;
        }
    }
...
- (BOOL)testStringValue:(NSString*)testValue forW3CProperty:(NSString*)property
{
    ...
    if ([self valueForKeyIsNumber:[CDVContact defaultW3CtoAB] key:property]) {
        ...
        if (ABPersonGetTypeOfProperty(propId) == kABStringPropertyType) {
            ...
            if ((propValue != nil) && ([propValue length] > 0)) {
                NSPredicate* containPred = [NSPredicate predicateWithFormat:@"SELF BEGINSWITH[cd] %@", testValue];
