 ...
 *	Filename : OnymosContactsManager.java
 *
 ...
 */
package com.onymos.components.contacts;
...
public class OnymosContactsManager extends CordovaPlugin {

		private OnymosContactsAccessor contactAccessor;
		private CallbackContext callbackContext;        // The callback context from which we were invoked.
		private JSONArray executeArgs;

		private static final String LOG_TAG = "OnymosContactsManager";
		...
		{
				OnymosPermissionHelper.requestPermission(this, requestCode, WRITE);
		}
		...
		public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
				...
				if (android.os.Build.VERSION.RELEASE.startsWith("1.")) {
						callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, OnymosContactsManager.NOT_SUPPORTED_ERROR));
						return true;
				}
				...
				if (this.contactAccessor == null) {
						this.contactAccessor = new OnymosContactsAccessorSDK5(this.cordova);
				}
				...
				if (action.equals("searchContacts")) {
						...
						if (OnymosPermissionHelper.hasPermission(this, WRITE))
						{
								save(executeArgs);
						}
						else
		...
		private void search(JSONArray args) throws JSONException
		{
				...
				final String searchTerm = options.optString("filter");

				this.cordova.getThreadPool().execute(new Runnable() {
						public void run() {
								JSONArray res = contactAccessor.search(filter, options);

								JSONObject usedSearchTermObject = new JSONObject();
								try {
									usedSearchTermObject.put("displayName", searchTerm);
									res.put(usedSearchTermObject);
								}
								catch (Exception e) {
		 ...
		 */
		private void selectContactAsync() {
